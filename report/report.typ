...
Παρατηρούμε ότι το read throughput για το Eventual Consistency αυξάνεται με την αύξηση του replication factor, γεγονός που είναι αναμενόμενο αφού μπορούν να διαβαστούν από οποιονδήποτε replica manager και όχι μόνο από τον primary node(και στις 2 υλοποίησεις). Αντίθετα για το Chain Replication, το read throughput μειώνεται με την αύξηση του replication factor, καθώς η ανάγνωση γίνεται από την ουρά της αλυσίδας ενώ παράλληλα δεν γίνονται εισαγωγές που αφορούν την αλυσίδα του υπέυθυνου κόμβου. Αξιοσημείωτο είναι ότι για k=1 το Chain Replication πετυχαίνει το καλύτερο read throughput. γεγονός που μπορεί να οφείλεται σε ευνοϊκότερη σειρά εκτέλεσης των ερωτημάτων σε συνδυασμό με την απουσία overhead που επιφέρει η ανάγνωση σε όλη την αλυσίδα(0 replicas για k=1).
